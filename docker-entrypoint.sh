...
php artisan tinker --execute="try { DB::connection()->getPdo(); echo 'DB Connected Successfully'; } catch(\Exception \$e) { echo 'DB Connection Failed: ' . \$e->getMessage(); exit(1); }" || {
    echo "ERROR: Cannot connect to database. Check these:"
    echo "  - DB_HOST, DB_PORT, DB_USERNAME, DB_PASSWORD env vars"
    echo "  - SSL configuration (MYSQL_ATTR_SSL_CA)"
